...
#if (SEV_DBG_EN == 1)
#define SEV_DBG(fmt, ...)		__LOG__(fmt, "SEV_DBG", ##__VA_ARGS__)
#else
#define SEV_DBG(fmt, ...)
#endif

#if (API_DBG_EN == 1)
#define API_DBG(fmt, ...)		__LOG__(fmt, "API_DBG", ##__VA_ARGS__)
#else
#define API_DBG(fmt, ...)
#endif

#if (CMD_DBG_EN == 1)
#define CMD_DBG(fmt, ...)		__LOG__(fmt, "CMD_DBG", ##__VA_ARGS__)
#else
#define CMD_DBG(fmt, ...)
#endif

#if (CCL_DBG_EN == 1)
#define CCL_DBG(fmt, ...)		__LOG__(fmt, "CCL_DBG", ##__VA_ARGS__)
#else
#define CCL_DBG(fmt, ...)
#endif
